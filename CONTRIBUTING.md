...
If you want to use Android Studio or Xcode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/AppUpdatesExample.xcworkspace` in Xcode and find the source files at `Pods > Development Pods > @sapkalabs/react-native-app-updates`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `@sapkalabs/react-native-app-updates` under `Android`.
...
```sh
Running "AppUpdatesExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
...
Merging a pull request into `main` now triggers an automated release flow. By
default, merged pull requests publish a patch release.

You can control the semantic version bump with one of these pull request
labels:
